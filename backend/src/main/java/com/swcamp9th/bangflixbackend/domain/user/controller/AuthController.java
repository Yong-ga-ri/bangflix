...
public class AuthController {
    ...
    public ResponseEntity<SuccessResponse<SignupResponseDto>> signup(@Valid @RequestPart(value = "signupDto") SignupRequestDto signupRequestDto, @RequestPart(value = "imgFile", required = false) MultipartFile imgFile) throws IOException {
        ...
        return ResponseEntity.ok(new SuccessResponse<>(200, "회원 가입 성공", null));
    }

    @PostMapping("/login")
    @Operation(summary = "로그인 API")
    public ResponseEntity<SuccessResponse<SignResponseDto>> login(@Valid @RequestBody SignRequestDto signRequestDto) {
        return ResponseEntity.ok(new SuccessResponse<>(200, "로그인 성공", userService.login(signRequestDto)));
    }
    ...
    public ResponseEntity<SuccessResponse<ReissueTokenResponseDto>> refresh(@Valid @RequestBody RefreshTokenRequestDto refreshTokenRequestDto) {
        return ResponseEntity.ok(new SuccessResponse<>(200, "엑세스 토큰 재발급 성공", userService.refreshTokens(refreshTokenRequestDto.getRefreshToken())));
    }
    ...
    public ResponseEntity<SuccessResponse<DuplicateCheckResponseDto>> confirmId(@Valid @RequestBody ConfirmIdRequestDto confirmIdRequestDto) {
        ...
        return ResponseEntity.ok(new SuccessResponse<>(200, "사용 가능한 아이디입니다", result));
    }
    ...
    public ResponseEntity<SuccessResponse<DuplicateCheckResponseDto>> confirmNickname(@Valid @RequestBody ConfirmNicknameRequestDto confirmNicknameRequestDto) {
        ...
        return ResponseEntity.ok(new SuccessResponse<>(200, "사용 가능한 닉네임입니다", result));
    }

    @PostMapping("/send-email")
    @Operation(summary = "인증 이메일 발송 API")
    public ResponseEntity<SuccessResponse<Object>> sendEmail(@RequestBody EmailRequestDto emailRequestDto) {
        emailService.sendSimpleMessage(emailRequestDto.getEmail());
        return ResponseEntity.ok(new SuccessResponse<>(200, "인증 이메일 발송에 성공했습니다", null));
    }

    @PostMapping("/confirm-email")
    @Operation(summary = "인증 이메일 검증 API")
    public ResponseEntity<SuccessResponse<Object>> confirmEmail(@RequestBody EmailCodeRequestDto emailCodeRequestDto) {
        emailService.findEmailCode(emailCodeRequestDto);
        return ResponseEntity.ok(new SuccessResponse<>(200, "이메일 인증에 성공했습니다", null));
